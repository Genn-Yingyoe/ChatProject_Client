...
namespace Test
{
    ...
    internal class DB_IO
    {
                    ...
                    while (true)                // 한 연결에서 계속 수신
                    {
                        ...
                        else if (opcode == 1)       //register user     |   items = (User_id(not use), Id,Ps,Ps_question_index,Ps_ans,Name,Nickname)    | test success
                        {
                            ...
                            else
                            {
                                ...
                                make_user.Ps_Question_Index = int.Parse(items[3]);
                        ...
                        else if (opcode == 3)       //find password     |   items = (User_id(not use),Id,Ps_index,Ps_ans,New_ps)        | test success
                        {
                            ...
                            {
                                if (user.Ps_Question_Index == int.Parse(items[2]) && user.Ps_Answer == items[3])
                                {
                                    //Ps_question success
                        ...
                        {
                            int inform_id = int.Parse(items[1]);
                            if (items[2] == "1")
                            {
                        ...
                        if (Add_temp.Count() != 0)
                        {
                            opcode_success = await SafeBatchAppendAsync(Add_temp);
                            if (!opcode_success)
                            {
                                //error
                                throw new Exception();
                        ...
                        if (Edit_str.Count() != 0)
                        {
                            opcode_success = await SafeBatchEditAsync(Edit_str, Edit_obj, Edit_int);
                            if (!opcode_success)
                            {
                                //error
                                throw new Exception();
                        ...
                        if (Del_str.Count() != 0)
                        {
                            opcode_success = await SafeBatchDeleteAsync(Del_str, Del_int);
                            if (!opcode_success)
                            {
                                //error
                                throw new Exception();
        ...
        */
        ...
        internal static async Task<int> LastInformId(string path)
        {
            int ans = -1;
            var ser = new DataContractJsonSerializer(typeof(_User_Id__Inform_Box));
