            ...
            else if (opcode == 38)      //read my_Chat_Room_List        |   itmes = {User_id}
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        {
                            // 입장한 채팅방 목록 추가
                            List<string> chat_room_list = user_info.Chat_Room_List;
                            foreach (string room in chat_room_list)
                            {
                                send_datas.Add(room + "|joined");  // 구분자를 통해 상태 표시
                            }

                            // 초대받은 채팅방 목록 추가
                            List<string> waiting_chat_room_list = user_info.Waiting_Chat_Room_List;
                            foreach (string room in waiting_chat_room_list)
                            {
                                send_datas.Add(room + "|invited");  // 구분자를 통해 상태 표시
                            }
