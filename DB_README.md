...
|--------|----------------------------|------------|-----------|
| 1      | register(회원가입)          | id, ps, ps_question_index, ps_question_answer, name(불변), nickname(가변) | 성공 1, 실패 0, 성공 시 User_Id(6자리) 발급 |
| 2      | login(로그인)               | id, ps     | 성공 시 User_Id, 1 추가 반환 / 실패 시 0 |
...
| 6      | friend_request(친구 요청)   | friend_id(User_Id, 6자리) | 성공 1 / 실패 0, Inform_Box 알림 추가 |
| 7      | friend_delete(친구 삭제)    | friend_id   | 성공 1 / 실패 0, DB 양쪽에서 삭제 |
| 8      | check_notify(알림 확인/수락)| Inform_Id, check_state("1":수락, "0":확인/거절) | 성공 1 / 실패 0  |
| 9      | delete_notify(알림 삭제)    | Inform_Id   | 읽은 알림만 삭제, 성공 1 / 실패 0 |
| 10     | read_friend_list(친구목록)  | 없음(빈 리스트) | 성공 시 Friend_List 전체 데이터 직렬화 전달, 실패 0 |
| 11     | read_all_of_notify(알림 전체 읽기) | 없음(빈 리스트) | 성공 시 모든 알림 직렬화 전달, 실패 0 |
| 12     | read_user_setting(유저 세팅 읽기) | 없음(빈 리스트) | 성공 시 Setting_Info 전체 직렬화 전달, 실패 0 |
...
| opcode | 기능                       | 필요 items         | 주의/설명 |
|--------|---------------------------|--------------------|-----------|
| 32     | make_chat_room(채팅방 만들기) | Friend_Id 1+명      | 성공 시 Room_Id, 1 반환. 실패 0 |
...
| 36     | write_chat(채팅 쓰기)         | Room_Id, chat_category, chat_str | 관리자 메세지는 category "0" 개발자 금지, 성공 1, 실패 0 |
| 37     | read_chat(채팅 읽기)          | Room_Id, last_read_msg_id, last_read_msg_date(yyyyMMdd) | 특수 상황 참고(새 PC, 등) |
| 38     | read_my_chat_room_list(내 채팅방 목록) | 없음(빈 리스트)    | 성공 시 Room_Id string들, 1 반환 |
| 39     | read_chat_room_users(채팅방 멤버 읽기) | Room_Id | 성공 시 직렬화된 멤버 데이터 전달, 실패 0 |
...
| 67     | read_user_sche(유저 스케쥴 읽기)          | 없음(빈 리스트) | 성공 시 "_User_Id__Scheduler" 형 직렬화 전달, 실패 0 |
...
| 71     | read_chatroom_sche(채팅방 스케쥴 읽기)    | Room_Id | 성공 시 직렬화 전달, 실패 0 |
...
// 직렬화/역직렬화 예시
string dataLine = @"{ ""Friend_Id"":""123123"", ""Nickname"":""Kwangwooni"" }";
...
{
    [DataMember] internal string User_Id;    // 6자리 | primary key
...
{
    [DataMember] internal string User_Id;     // PK, FK
...
{
    [DataMember] internal int Inform_Id;           // PK
    [DataMember] internal string Inform_Kind;      // 예: "friend_request", "invite"
...
{
    [DataMember] internal string Friend_Id;       // PK, FK
    [DataMember] internal string Nickname;
}
```
...
{
    [DataMember] internal string Info_Id;         // PK
...
{
    [DataMember] internal string Room_Id;         // 8자리, PK
    [DataMember] internal int Users_Num;          // 입장 중 유저수(invite_state true)
}
```
...
{
    [DataMember] internal string User_Id;         // 6자리, PK, FK
    [DataMember] internal int Read_Msg_Num;
    [DataMember] internal string Read_Last_Date;
    [DataMember] internal string Sche_List;
...
{
    [DataMember] internal int Msg_Id;             // PK
    [DataMember] internal string User_Id;         // FK
    [DataMember] internal int Msg_Kind;           // 0: 관리자메시지, 1:유저채팅, 기타
...
{
    [DataMember] internal int Sche_Id;            // PK
...
# 4. DB 및 Test_Client 사용법

1. 모든 관련 파일을 프로젝트 폴더에 정리  
    - `DB_IO.cs`, `DB_Data_Type.cs`, `ChatRoomClass.cs`, `SchedulerClass.cs`, `Program.cs`
2. Test_Client 파일은 **별도 프로젝트**에 위치  
    - `Form1.cs`, `Form1.cs [디자인]`
3. Visual Studio 2022에서 **ChatMoa_DataBaseServer**로 실행 (F5)
4. DBSETUP 메시지 확인 후 DB폴더 자동 생성
5. Test_Client 프로젝트의 Window Forms를 실행
6. 두 프로그램(서버 콘솔/클라이언트 창) 동시에 구동
7. Client에서 데이터 입력, 서버로 전송, 결과 MessageBox 확인
8. DB폴더 내 ndjson 파일(메모장 열림)로 변화 확인
9. 기본적으로 C드라이브에 DB폴더 생성
